...
async function load(){if(!token)return;try{const r=await fetch(location.origin+'/api/v2/owner/live/drivers',{headers:{Authorization:'Bearer '+token}});if(r.status===401||r.status===403){token='';document.getElementById('login').classList.remove('hidden');document.getElementById('controls').classList.add('hidden');document.getElementById('loginStatus').textContent='Owner session expired. Sign in again.';document.getElementById('loginStatus').className='error';return;}if(!r.ok)throw new Error('HTTP '+r.status);const body=await r.json();drivers=Array.isArray(body)?body:(body.drivers||body.items||[]);render();}catch(e){document.getElementById('status').textContent='Live data unavailable: '+(e&&e.message?e.message:'connection error');}}
async function signIn(){const status=document.getElementById('loginStatus');status.textContent='Signing in…';status.className='';const username=document.getElementById('username').value.trim(),password=document.getElementById('password').value;try{const r=await fetch(location.origin+'/api/v2/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username,password})});if(!r.ok){document.getElementById('password').value='';status.textContent=r.status===401?'Username or password is incorrect.':'Sign-in failed ('+r.status+').';status.className='error';return;}const b=await r.json();token=b.accessToken||b.access_token||'';if(!token){status.textContent='Sign-in response did not include an access token.';status.className='error';return;}document.getElementById('login').classList.add('hidden');document.getElementById('controls').classList.remove('hidden');await load();}catch(e){status.textContent='Cannot reach the Sterling API: '+(e&&e.message?e.message:'connection error');status.className='error';}}
