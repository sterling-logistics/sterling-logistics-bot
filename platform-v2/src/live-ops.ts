...
const eventSchema = z.object({
  eventType: z.enum(['game.started', 'game.stopped', 'job.started', 'job.delivered', 'fine', 'damage.changed', 'tracker.started', 'tracker.stopped']),
  game: z.enum(['ets2', 'ats']).nullable().optional(),
    ...
    await db.execute(
      `INSERT INTO driver_presence
        (user_id, tracker_version, game, game_running, on_job, status, last_seen_at, last_login_at,
         latitude, longitude, heading_deg, speed_kph, city, truck_make, truck_model, cargo,
       ...
       ON DUPLICATE KEY UPDATE
         ...
         latitude = VALUES(latitude), longitude = VALUES(longitude), heading_deg = VALUES(heading_deg),
      ...
      [user.id, d.trackerVersion ?? null, d.game ?? null, d.gameRunning ? 1 : 0, d.onJob ? 1 : 0, d.status,
       d.latitude ?? null, d.longitude ?? null, d.headingDeg ?? null, d.speedKph ?? null, d.city ?? null,
       d.truckMake ?? null, d.truckModel ?? null, d.cargo ?? null, d.originCity ?? null,
       d.destinationCity ?? null, d.fuelPercent ?? null, d.damagePercent ?? null, d.finesTotal ?? null]
    );
  ...
  app.get('/api/v2/owner/live/drivers', { preHandler: requireStaff(['owner']) }, async () => {
    const [rows] = await db.query<any[]>(
      `SELECT u.id, u.username, u.display_name AS displayName, u.role, u.rank_name AS rankName,
              ...
              p.latitude, p.longitude, p.heading_deg AS headingDeg, p.speed_kph AS speedKph,
