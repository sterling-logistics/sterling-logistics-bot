...
public partial class MainWindow : Window
{
    ...
    void SetSignedOutUi(){StatusText.Text="● Sterling account not connected";AccountText.Text="Sign in with Discord to link your approved Sterling driver profile";DriverNameText.Text="Not signed in";DriverIdText.Text="—";RankText.Text="—";ConnectionText.Text="Offline";AccountButton.Content="Sign in with Discord";}
    ...
    async Task Loop()
    {
        while(running)
        {
            try
            {
                ...
                if(!gameRunning){GameText.Text="ETS2 not detected";TelemetryText.Text="Waiting for game";ConnectionText.Text=string.IsNullOrWhiteSpace(sessionToken)?"Offline":"Connected";await Task.Delay(3000);continue;}
                GameText.Text="ETS2 detected";
                var json=await http.GetStringAsync(TelemetryUrl);using var doc=JsonDocument.Parse(json);var raw=doc.RootElement.Clone();
                if(!BoolAny(raw,"SdkActive")){TelemetryText.Text="SDK waiting";await Task.Delay(3000);continue;}
                UpdateUi(raw);TelemetryText.Text="Live telemetry active";
                if(!string.IsNullOrWhiteSpace(sessionToken))await DetectAndSend(raw);
            }
    ...
    async Task DetectAndSend(JsonElement d)
    {
        ...
        JobStateText.Text=eventType=="heartbeat"?(onJob?"Active":"Waiting"):$"{eventType.Replace('-',' ')}";
    }
    ...
    void UpdateUi(JsonElement d)
    {
        ...
        var limit=NumAny(d,"NavigationValues.SpeedLimit.Mph");SpeedLimitText.Text=limit>0?$"{limit:0} mph":"—";
        FuelText.Text=$"{NumAny(d,"TruckValues.CurrentValues.DashboardValues.FuelValue.Amount"):0} L";
        ...
        var src=StrAny(d,"JobValues.CitySource"),dst=StrAny(d,"JobValues.CityDestination");RouteText.Text=$"{(src.Length>0?src:"—")}  →  {(dst.Length>0?dst:"—")}";
        var km=NumAny(d,"JobValues.PlannedDistanceKm");DistanceText.Text=km>0?$"{km*0.621371:0} mi":"—";
        var income=NumAny(d,"JobValues.Income");JobValueText.Text=income>0?$"£{income:N0}":"—";
        JobStateText.Text=BoolAny(d,"SpecialEventsValues.OnJob")?"Active":"Waiting";
    }
