...
public partial class MainWindow : Window
{
    ...
    readonly HttpClient telemetryHttp = new() { Timeout = TimeSpan.FromMilliseconds(900) };
    ...
    async void AccountButton_Click(object sender, RoutedEventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(sessionToken))
        {
            ...
            ClearToken();
            FooterText.Text = "Signed out of Sterling";
            return;
        }
        ...
        try
        {
            StatusText.Text = "Opening Discord sign in";
            FooterText.Text = "Complete the secure login in your browser";
            ...
            if (!res.IsSuccessStatusCode) throw new Exception(ReadError(raw));
    ...
    void SetSignedOutUi()
    {
        StatusText.Text = "Sterling account not connected";
        AccountText.Text = "Sign in with Discord to link your approved Sterling driver profile";
        DriverNameText.Text = "Not signed in";
        DriverIdText.Text = "—";
        RankText.Text = "—";
        ConnectionText.Text = "Offline";
        AccountButton.Content = "Sign in with Discord";
    }
    ...
    async Task LiveLoop()
    {
        while (running)
        {
            ...
            try
            {
                if ((DateTime.UtcNow - lastGameCheckAt).TotalMilliseconds >= 1000)
                {
                ...
                if (!gameRunning)
                {
                    ...
                    TelemetryText.Text = "Waiting for game";
                ...
                var json = await telemetryHttp.GetStringAsync(TelemetryUrl);
                started.Stop();
                ...
                if (!BoolAny(raw, "SdkActive"))
                {
                    ...
                    PingText.Text = $"{started.ElapsedMilliseconds} ms";
                    await Task.Delay(350);
                    continue;
                }
                ...
                if (!string.IsNullOrWhiteSpace(sessionToken) && !uploadBusy && (DateTime.UtcNow - lastUploadAt).TotalMilliseconds >= 1000)
                {
            ...
            var remaining = 200 - (int)started.ElapsedMilliseconds;
    ...
    async Task DetectAndSend(JsonElement d)
    {
        ...
        {
            { "JobDelivered", "job-delivered" }, { "JobCancelled", "job-cancelled" }, { "Refuel", "refuel" },
            { "RefuelPayed", "refuel-paid" }, { "Fined", "fine" }, { "Tollgate", "toll" }, { "Ferry", "ferry" }, { "Train", "train" }
        };

        foreach (var pair in eventMap)
        {
            var now = BoolAny(d, $"SpecialEventsValues.{pair.Key}");
            if (now && !lastFlags[pair.Key]) { eventType = pair.Value; direct = true; break; }
        }
        if (!direct && gameTimeJump >= 120) { eventType = "rest-stop"; direct = true; }
        if (!direct)
        {
            if (onJob && !lastOnJob) { eventType = "job-started"; direct = true; }
            else if (!onJob && lastOnJob) { eventType = "job-ended"; direct = true; }
        }
    ...
    async Task SendTelemetry(JsonElement d, string eventType, bool direct, double gameTimeJump)
    {
        ...
        {
            ["game"] = StrAny(d, "Game"), ["paused"] = BoolAny(d, "Paused"), ["sdkActive"] = BoolAny(d, "SdkActive"),
            ["speedMps"] = speed, ["speedLimitMph"] = NumAny(d, "NavigationValues.SpeedLimit.Mph"),
            ...
            ["odometerKm"] = NumAny(d, "TruckValues.CurrentValues.DashboardValues.Odometer"), ["truckDamage"] = MaxDamage(d),
            ["trailerDamage"] = NumAny(d, "TrailerValues.0.DamageValues.Body"), ["cargoDamage"] = FirstNum(d, "JobValues.CargoValues.CargoDamage", "GamePlay.JobDelivered.CargoDamage"),
            ["engineOn"] = BoolAny(d, "TruckValues.CurrentValues.EngineEnabled"), ["engineRpm"] = NumAny(d, "TruckValues.CurrentValues.DashboardValues.RPM"),
            ["gameTime"] = NumAny(d, "CommonValues.GameTime.Value"), ["gameTimeJump"] = gameTimeJump,
            ["latitude"] = NullableNum(d, "TruckValues.CurrentValues.PositionValue.X"), ["longitude"] = NullableNum(d, "TruckValues.CurrentValues.PositionValue.Z"),
            ["onJob"] = BoolAny(d, "SpecialEventsValues.OnJob"), ["fineAmount"] = NumAny(d, "GamePlay.FinedEvent.Amount"), ["fineOffence"] = StrAny(d, "GamePlay.FinedEvent.Offence")
        };
        ...
        FooterText.Text = direct ? $"Sterling recorded {eventType.Replace('-', ' ')}" : "Live data streaming to Sterling";
    ...
    void UpdateUi(JsonElement d)
    {
        ...
        var limit = NumAny(d, "NavigationValues.SpeedLimit.Mph");
        SpeedLimitText.Text = limit > 0 ? $"{limit:0} mph" : "—";
        var rpm = NumAny(d, "TruckValues.CurrentValues.DashboardValues.RPM");
        RpmText.Text = rpm > 0 ? $"{rpm:0}" : "0";
        var fuel = NumAny(d, "TruckValues.CurrentValues.DashboardValues.FuelValue.Amount");
        FuelText.Text = $"{fuel:0} L";
        DamageText.Text = $"{MaxDamage(d) * 100:0.0}%";
        var gear = FirstNum(d, "TruckValues.CurrentValues.DashboardValues.GearDashboards", "TruckValues.CurrentValues.DashboardValues.Gear");
        GearText.Text = gear == 0 ? "N" : gear.ToString("0");
        ...
        TruckText.Text = truck.Length > 0 ? truck : "No truck detected";
        ...
        var km = NumAny(d, "JobValues.PlannedDistanceKm");
        DistanceText.Text = km > 0 ? $"{km * 0.621371:0} mi" : "—";
        var income = NumAny(d, "JobValues.Income");
        JobValueText.Text = income > 0 ? $"£{income:N0}" : "—";
        DriverShareText.Text = income > 0 ? $"£{income * 0.35:N0}" : "—";
    ...
    HttpRequestMessage Authorized(HttpMethod method, string path)
    {
        ...
        if (!string.IsNullOrWhiteSpace(sessionToken)) req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", sessionToken);
        return req;
    }

    static double SpeedMps(JsonElement d)
    {
        var speed = NumAny(d, "TruckValues.CurrentValues.DashboardValues.Speed.Value");
        if (speed == 0)
        {
            var kph = NumAny(d, "TruckValues.CurrentValues.DashboardValues.Speed.Kph");
            if (kph != 0) speed = kph / 3.6;
        }
        return speed;
    }

    static string ReadError(string json)
    {
        try { using var d = JsonDocument.Parse(json); return d.RootElement.TryGetProperty("error", out var e) ? e.GetString() ?? "Sterling login failed" : "Sterling login failed"; }
        catch { return "Sterling login failed"; }
    }

    static string First(params string[] x) => x.FirstOrDefault(v => !string.IsNullOrWhiteSpace(v)) ?? "";
    static JsonElement? At(JsonElement e, string path)
    {
        foreach (var p in path.Split('.'))
        {
            if (e.ValueKind == JsonValueKind.Array && int.TryParse(p, out var i)) { if (i < 0 || i >= e.GetArrayLength()) return null; e = e[i]; continue; }
            if (e.ValueKind != JsonValueKind.Object || !e.TryGetProperty(p, out var n)) return null;
            e = n;
        }
        return e;
    }
