...
import {ensureTrackerEventsChannel,postTrackerEvent} from "./telemetry/events.js";
...
app.post("/api/tracker/telemetry",async(q,r)=>{try{if(!schemaReady)return r.status(503).json({ok:false,error:"Database schema is still starting"});const auth=String(q.headers.authorization||"");const token=auth.startsWith("Bearer ")?auth.slice(7):"";const driver=await authenticateTracker(token);if(!driver)return r.status(401).json({ok:false,error:"Invalid tracker key"});const out=await ingestTrackerTelemetry(driver.driver_id,q.body||{});if(out.sessionBecameOnline)postTrackerEvent(client,c.guildId,driver,{type:"driver-online",data:q.body?.data||{}}).catch(e=>console.error("[Tracker Presence]",e));r.json({...out,driver:driver.sterling_driver_id});}catch(e){console.error("[Tracker API]",e);r.status(400).json({ok:false,error:String(e.message||e)})}});
app.listen(c.port,"0.0.0.0",()=>console.log(`[API] Listening on ${c.port}`));
client.once(Events.ClientReady,async me=>{console.log(`[Discord] Logged in as ${me.user.tag}`);try{const d=await pingDatabase();console.log(`[DB] Connected to ${d.db} as ${d.username}`);await ensureSchema();await ensureDispatchSchema();schemaReady=true;console.log("[DB] MySQL schema ready");const g=await client.guilds.fetch(c.guildId);await ensureTrackerEventsChannel(g);const x=await reconcileTickets(g);console.log(`[Tickets] Startup repair: ${x.valid} valid, ${x.stale} stale, ${x.duplicates} duplicate repaired`);await registerCommands(c);console.log("[Sterling] Bot ready");}catch(e){schemaReady=false;console.error("[Startup]",e);}});client.on(Events.ChannelDelete,handleManualChannelDelete);

setInterval(async()=>{if(!schemaReady||!client.isReady())return;try{const offline=await markStaleTrackerSessionsOffline(90);for(const d of offline)await postTrackerEvent(client,c.guildId,d,{type:"driver-offline",data:d.data,lastSeenAt:d.last_seen_at});}catch(e){console.error("[Tracker Presence Sweep]",e);}},30000);
