...
app.get("/health",async(_q,r)=>{try{const d=await pingDatabase();r.json({ok:true,discord:client.isReady(),database:d.db});}catch(e){r.status(500).json({ok:false,error:String(e.message||e)})}});
