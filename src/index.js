...
app.post("/api/tracker/telemetry",async(q,r)=>{try{if(!schemaReady)return r.status(503).json({ok:false,error:"Database schema is still starting"});const auth=String(q.headers.authorization||"");const token=auth.startsWith("Bearer ")?auth.slice(7):"";const driver=await authenticateTracker(token);if(!driver)return r.status(401).json({ok:false,error:"Invalid tracker key"});const body=q.body||{};const data=body.data||{};const eventType=String(body.eventType||"heartbeat");const out=await ingestTrackerTelemetry(driver.driver_id,body);if(out.sessionBecameOnline)postTrackerPresence(client,c.guildId,driver,{type:"driver-online",data}).catch(e=>console.error("[Tracker Presence]",e));let event=null;if(["job-started","job-delivered","job-cancelled","fine","rest-stop","toll","ferry","train"].includes(eventType))event={type:eventType,data};else if(out.metrics?.crashDetected)event={type:"crash",data,speedMph:Number(data.speedMps||0)*2.2369362921,damageDelta:out.metrics.damageDelta||0};else if(Number(out.metrics?.fuelAdded||0)>0)event={type:"fuel-stop",data,fuelAdded:out.metrics.fuelAdded};if(event)postTrackerEvent(client,c.guildId,driver,event).catch(e=>console.error("[Tracker Events]",e));r.json({...out,driver:driver.sterling_driver_id});}catch(e){console.error("[Tracker API]",e);r.status(400).json({ok:false,error:String(e.message||e)})}});
