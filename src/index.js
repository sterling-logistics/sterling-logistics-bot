...
app.post("/api/tracker/telemetry",async(q,r)=>{try{const auth=String(q.headers.authorization||"");const token=auth.startsWith("Bearer ")?auth.slice(7):"";const driver=await authenticateTracker(token);if(!driver)return r.status(401).json({ok:false,error:"Invalid tracker key"});const out=await ingestTrackerTelemetry(driver.driver_id,q.body||{});const eventType=String(q.body?.eventType||"heartbeat");const data=q.body?.data||{};let event=null;if(["job-started","job-delivered","rest-stop"].includes(eventType))event={type:eventType,data};else if(out.metrics?.crashDetected)event={type:"crash",data,speedMph:Number(data.speedMps||0)*2.2369362921,damageDelta:out.metrics.damageDelta||0};else if(Number(out.metrics?.fuelAdded||0)>0)event={type:"fuel-stop",data,fuelAdded:out.metrics.fuelAdded};if(event)postTrackerEvent(client,c.guildId,driver,event).catch(e=>console.error("[Tracker Events]",e));r.json({...out,driver:driver.sterling_driver_id});}catch(e){console.error("[Tracker API]",e);r.status(400).json({ok:false,error:String(e.message||e)})}});
app.listen(c.port,"0.0.0.0",()=>console.log(`[API] Listening on ${c.port}`));
client.once(Events.ClientReady,async me=>{console.log(`[Discord] Logged in as ${me.user.tag}`);try{const d=await pingDatabase();console.log(`[DB] Connected to ${d.db} as ${d.username}`);await ensureSchema();await ensureDispatchSchema();console.log("[DB] MySQL schema ready");const g=await client.guilds.fetch(c.guildId);await ensureTrackerEventsChannel(g);const x=await reconcileTickets(g);console.log(`[Tickets] Startup repair: ${x.valid} valid, ${x.stale} stale, ${x.duplicates} duplicate repaired`);await registerCommands(c);await registerDispatchCommands(c);console.log("[Sterling] Bot ready");}catch(e){console.error("[Startup]",e);}});client.on(Events.ChannelDelete,handleManualChannelDelete);
