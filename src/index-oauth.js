...
import {registerGoogleApplicationRoutes,handleGoogleApplicationButton,handleGoogleApplicationDeclineModal} from "./recruitment/google.js";
...
import {registerWebsiteRoutes,ensureWebsiteSchema} from "./web/site.js";
import {handleSignOn,handleSignOff,handleDutyStatus} from "./duty/service.js";
...
if(c.enableMessageContentIntent)intents.push(GatewayIntentBits.MessageContent);
const client=new Client({intents});
const app=express();
let schemaReady=false;
app.use(express.json({limit:"512kb"}));
registerDesktopAuthRoutes(app,c);
registerWebsiteRoutes(app,c);
registerGoogleApplicationRoutes(app,client,c);

async function trackerAuth(q){const auth=String(q.headers.authorization||"");const token=auth.startsWith("Bearer ")?auth.slice(7):"";return(await authenticateTracker(token))||(await authenticateDesktopSession(token));}
registerDispatchStaffRoutes(app,trackerAuth,{includeAssignments:true});

app.get("/health",async(_q,r)=>{try{const d=await pingDatabase();r.json({ok:true,discord:client.isReady(),database:d.db,schemaReady,desktopLogin:Boolean(c.discordClientSecret&&c.publicBaseUrl),websiteLogin:Boolean(c.discordClientSecret&&c.publicBaseUrl),dispatchApi:true,staffJobApprovals:true,staffPayouts:true});}catch(e){r.status(500).json({ok:false,error:String(e.message||e)})}});
...
app.get("/api/desktop/me",async(q,r)=>{try{if(!schemaReady)return r.status(503).json({ok:false,error:"Database schema is still starting"});const d=await trackerAuth(q);if(!d)return r.status(401).json({ok:false,error:"Session expired"});r.json({ok:true,driver:{sterlingDriverId:d.sterling_driver_id,discordUsername:d.discord_username,rank:d.rank_name||null,totalMiles:Number(d.total_miles||0),jobsCompleted:Number(d.jobs_completed||0)}});}catch(e){r.status(400).json({ok:false,error:String(e.message||e)})}});
app.get("/api/tracker/payout",async(q,r)=>{try{if(!schemaReady)return r.status(503).json({ok:false,error:"Database schema is still starting"});const d=await trackerAuth(q);if(!d)return r.status(401).json({ok:false,error:"Invalid tracker session"});r.json({ok:true,payout:await getPendingEts2Payout(d.driver_id)});}catch(e){r.status(400).json({ok:false,error:String(e.message||e)})}});
app.post("/api/tracker/payout/:id/complete",async(q,r)=>{try{const d=await trackerAuth(q);if(!d)return r.status(401).json({ok:false});r.json({ok:await completeEts2Payout(d.driver_id,Number(q.params.id),q.body?.savePath)});}catch(e){r.status(400).json({ok:false,error:String(e.message||e)})}});
app.post("/api/tracker/payout/:id/fail",async(q,r)=>{try{const d=await trackerAuth(q);if(!d)return r.status(401).json({ok:false});await failEts2Payout(d.driver_id,Number(q.params.id),q.body?.error);r.json({ok:true});}catch(e){r.status(400).json({ok:false,error:String(e.message||e)})}});
app.post("/api/tracker/telemetry",async(q,r)=>{try{if(!schemaReady)return r.status(503).json({ok:false,error:"Database schema is still starting"});const driver=await trackerAuth(q);if(!driver)return r.status(401).json({ok:false,error:"Invalid tracker session"});const body=q.body||{},data=body.data||{},eventType=String(body.eventType||"heartbeat");const out=await ingestTrackerTelemetry(driver.driver_id,body);let persistedJob=null;if(["job-started","job-delivered","job-cancelled"].includes(eventType)){try{persistedJob=await persistTrackerJobEvent(driver.driver_id,body);}catch(e){console.error("[Job Persistence]",e);}}if(out.sessionBecameOnline)postTrackerPresence(client,c.guildId,driver,{type:"driver-online",data}).catch(e=>console.error("[Tracker Presence]",e));let event=null;if(["job-started","job-delivered","job-cancelled","fine","rest-stop","toll","ferry","train"].includes(eventType))event={type:eventType,data};else if(out.metrics?.crashDetected)event={type:"crash",data,speedMph:Number(data.speedMps||0)*2.2369362921,damageDelta:out.metrics.damageDelta||0};else if(Number(out.metrics?.fuelAdded||0)>0)event={type:"fuel-stop",data,fuelAdded:out.metrics.fuelAdded};if(event)postTrackerEvent(client,c.guildId,driver,event).catch(e=>console.error("[Tracker Events]",e));r.json({...out,persistedJob,driver:driver.sterling_driver_id});}catch(e){console.error("[Tracker API]",e);r.status(400).json({ok:false,error:String(e.message||e)})}});

app.listen(c.port,"0.0.0.0",()=>console.log(`[API] Listening on ${c.port} • Dispatch Staff API enabled`));

client.once(Events.ClientReady,async me=>{console.log(`[Discord] Logged in as ${me.user.tag}`);try{const d=await pingDatabase();console.log(`[DB] Connected to ${d.db} as ${d.username}`);await ensureSchema();await ensureDispatchSchema();await ensureEconomySchema();await ensureWebsiteSchema();await ensureDispatchStaffApiSchema();schemaReady=true;console.log("[DB] MySQL schema ready");console.log("[Dispatch] Staff API ready • assignments + approvals + payouts");const g=await client.guilds.fetch(c.guildId);await ensureTrackerChannels(g);const x=await reconcileTickets(g);console.log(`[Tickets] Startup repair: ${x.valid} valid, ${x.stale} stale, ${x.duplicates} duplicate repaired`);await registerCommands(c);console.log(`[Sterling] Bot ready${c.discordClientSecret&&c.publicBaseUrl?" • Discord desktop + website login enabled":""}`);}catch(e){schemaReady=false;console.error("[Startup]",e);}});
client.on(Events.ChannelDelete,handleManualChannelDelete);
...
client.on(Events.InteractionCreate,async i=>{try{
  if(i.isModalSubmit()){
    ...
    if(i.customId.startsWith("googleapp_decline_modal:"))return handleGoogleApplicationDeclineModal(i);
  }
  if(i.isButton()){
    ...
    if(i.customId.startsWith("googleapp_"))return handleGoogleApplicationButton(i);
    return;
  }
  ...
  if(i.commandName==="signon")return handleSignOn(i);
  if(i.commandName==="signoff")return handleSignOff(i);
  if(i.commandName==="dutystatus")return handleDutyStatus(i);
  ...
  const staff=new Set(["claim","ticketinfo","adduser","removeuser","rename","transcript","closeticket"]);
  if(staff.has(i.commandName)&&!isTicketStaff(i,c))return i.reply({content:"Only authorised Sterling Logistics staff can use this command.",flags:MessageFlags.Ephemeral});
  switch(i.commandName){case"claim":return handleClaim(i);case"ticketinfo":return handleTicketInfo(i);case"adduser":return handleAddUser(i);case"removeuser":return handleRemoveUser(i);case"rename":return handleRename(i);case"transcript":return handleTranscript(i,client,c);case"closeticket":return handleCloseTicket(i,client,c);}
}catch(e){console.error("[Interaction]",e);const m=`Sterling Logistics Bot error\n\n${String(e.message||e).slice(0,1000)}`;try{if(i.deferred)await i.editReply(m);else if(i.replied)await i.followUp({content:m,flags:MessageFlags.Ephemeral});else await i.reply({content:m,flags:MessageFlags.Ephemeral});}catch{}}});

client.on(Events.Error,e=>console.error("[Discord]",e));
process.on("unhandledRejection",e=>console.error("[Node]",e));
process.on("uncaughtException",e=>console.error("[Node]",e));
client.login(c.token).catch(e=>{console.error("[Discord] Login failed",e);process.exit(1);});