...
export async function ingestTrackerTelemetry(driverId,p){
  ...
  let prevData={};try{prevData=typeof prev?.raw_json==="string"?JSON.parse(prev.raw_json):(prev?.raw_json||{});}catch{}

  const speedMps=num(data.speedMps??data.speed),speedMph=speedMps*2.2369362921;
  const fuel=num(data.fuelLiters),prevFuel=num(prevData.fuelLiters);
  const truckDamage=num(data.truckDamage),prevTruckDamage=num(prevData.truckDamage);
  const odometerKm=num(data.odometerKm),prevOdometerKm=num(prevData.odometerKm);
  const engineOn=Boolean(data.engineOn);
  let elapsed=0;if(prev?.last_seen_at)elapsed=Math.max(0,Math.min(30,Math.round((Date.now()-new Date(prev.last_seen_at).getTime())/1000)));
  ...
  if(fuelAdded>0){const[fr]=await db().execute("INSERT INTO fuel_stops(driver_id,session_id,liters_added,fuel_before,fuel_after,latitude,longitude) VALUES(?,?,?,?,?,?,?)",[driverId,sessionId,fuelAdded,prevFuel,fuel,data.latitude??null,data.longitude??null]);try{await recordFuelExpense(driverId,fuelAdded,sessionId,fr.insertId);}catch(e){console.error("[Economy Fuel]",e);}}
  ...
  return{ok:true,sessionId,sessionBecameOnline,metrics:{elapsed,milesDelta,fuelUsed,fuelAdded,crashDetected,damageDelta},dispatch,economy,progression};
}
...
export async function markStaleTrackerSessionsOffline(staleSeconds=90){
  ...
  const[r]=await db().query(`SELECT ts.driver_id,MAX(ts.last_seen_at) last_seen_at,d.sterling_driver_id,d.discord_id,d.discord_username,lt.raw_json FROM telemetry_sessions ts JOIN drivers d ON d.id=ts.driver_id LEFT JOIN live_telemetry lt ON lt.driver_id=ts.driver_id WHERE ts.status<>'offline' GROUP BY ts.driver_id,d.sterling_driver_id,d.discord_id,d.discord_username,lt.raw_json HAVING MAX(ts.last_seen_at)<DATE_SUB(NOW(),INTERVAL ${seconds} SECOND)`);
  const out=[];for(const x of r){await db().execute(`UPDATE telemetry_sessions SET status='offline',ended_at=COALESCE(ended_at,NOW()) WHERE driver_id=? AND status<>'offline' AND last_seen_at<DATE_SUB(NOW(),INTERVAL ${seconds} SECOND)`,[x.driver_id]);await db().execute("UPDATE live_telemetry SET status='offline' WHERE driver_id=?",[x.driver_id]);let data={};try{data=typeof x.raw_json==="string"?JSON.parse(x.raw_json):(x.raw_json||{});}catch{}out.push({...x,data});}return out;
}
...
export async function getLiveFleet(){const[r]=await db().query(`SELECT lt.*,d.sterling_driver_id,d.discord_username,d.rank_name,dm.driving_seconds,dm.crashes,dm.fuel_stops FROM live_telemetry lt JOIN drivers d ON d.id=lt.driver_id LEFT JOIN driver_metrics dm ON dm.driver_id=d.id WHERE lt.status<>'offline' AND lt.last_seen_at >= (NOW()-INTERVAL 2 MINUTE) ORDER BY lt.last_seen_at DESC`);return r;}

export async function handleDrivingStats(i){
  const u=i.options.getUser("user")||i.user;
  const[r]=await db().execute(`SELECT d.id,d.sterling_driver_id,d.discord_id,d.discord_username,d.rank_name,d.total_miles,d.jobs_completed,d.safety_score,dm.* FROM drivers d LEFT JOIN driver_metrics dm ON dm.driver_id=d.id WHERE d.discord_id=? LIMIT 1`,[u.id]);
  const x=r[0];if(!x)return i.reply({content:"No Sterling driver profile found.",flags:MessageFlags.Ephemeral});
  const[f,score,econ]=await Promise.all([db().execute("SELECT COUNT(*) incidents FROM driver_incidents WHERE driver_id=? AND occurred_at>=DATE_FORMAT(NOW(),'%Y-%m-01')",[x.id]),calculateDriveScore(x.id),getDriverEconomy(x.id)]);
  await i.reply({embeds:[new EmbedBuilder().setTitle(`🚛 Driving Stats | ${x.sterling_driver_id}`).addFields(
    {name:"Rank",value:x.rank_name||"Trainee Driver",inline:true},{name:"DriveScore",value:`${score.score.toFixed(0)}/100`,inline:true},{name:"Driving Hours",value:hours(x.driving_seconds),inline:true},
    {name:"Online Hours",value:hours(x.total_online_seconds),inline:true},{name:"Tracked Miles",value:Number(x.tracked_miles||0).toLocaleString(undefined,{maximumFractionDigits:1}),inline:true},{name:"Fuel Used",value:`${Number(x.fuel_used_liters||0).toFixed(1)} L`,inline:true},
