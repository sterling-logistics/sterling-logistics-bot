...
function roleMatches(guild){
  return guild.roles.cache.filter(r=>STAFF_ROLE_NAMES.some(n=>r.name.toLowerCase()===n.toLowerCase()));
}

async function staffCategory(guild){
  await guild.channels.fetch();
  return guild.channels.cache.find(c=>c.type===ChannelType.GuildCategory&&c.name.toLowerCase()===STAFF_CATEGORY_NAME)||null;
}

function hiddenOverwrites(guild,roles){
  return [
    {id:guild.roles.everyone.id,deny:[PermissionFlagsBits.ViewChannel]},
    ...roles.map(r=>({id:r.id,allow:[PermissionFlagsBits.ViewChannel,PermissionFlagsBits.ReadMessageHistory,PermissionFlagsBits.SendMessages]}))
  ];
}

async function applyHiddenStaffAccess(ch,guild,roles,parent){
  if(parent&&ch.parentId!==parent.id)await ch.setParent(parent.id,{lockPermissions:false}).catch(()=>{});
  for(const o of hiddenOverwrites(guild,roles)){
    await ch.permissionOverwrites.edit(o.id,{ViewChannel:o.deny?false:true,ReadMessageHistory:o.deny?undefined:true,SendMessages:o.deny?undefined:true}).catch(()=>{});
  }
}
...
export async function ensureTrackerChannels(guild){
  ...
  await guild.channels.fetch();
  const roles=[...roleMatches(guild).values()];
  const parent=await staffCategory(guild);
  ...
  if(!events){
    events=await guild.channels.create({name:EVENT_CHANNEL_NAME,type:ChannelType.GuildText,parent:parent?.id,topic:"Hidden Sterling tracker event log for management.",permissionOverwrites:hiddenOverwrites(guild,roles)});
  }
  await applyHiddenStaffAccess(events,guild,roles,parent);
  if(events.topic!=="Hidden Sterling tracker event log for management.")await events.setTopic("Hidden Sterling tracker event log for management.").catch(()=>{});
  trackerEventsChannelId=events.id;
  ...
  if(!status){
    status=await guild.channels.create({name:STATUS_CHANNEL_NAME,type:ChannelType.GuildText,parent:parent?.id,topic:"Staff alerts when Sterling drivers connect to or disconnect from ETS2 tracking.",permissionOverwrites:hiddenOverwrites(guild,roles)});
  }
  await applyHiddenStaffAccess(status,guild,roles,parent);
  driverStatusChannelId=status.id;
...
export async function postTrackerPresence(client,guildId,driver,event){
  ...
  try{
    ...
    const setup=await ensureTrackerChannels(guild);
    const ch=driverStatusChannelId?await guild.channels.fetch(driverStatusChannelId).catch(()=>null):setup.status;
    const d=event.data||{};const online=event.type==="driver-online";
    const title=online?"🟢 Driver Loaded ETS2":"🔴 Driver Left ETS2";
    const description=online?`**${who(driver)}** is now connected to the Sterling Tracker.`:`**${who(driver)}** is no longer connected to the Sterling Tracker.`;
    const fields=[];
    ...
    await ch.send({content:ping||undefined,embeds:[e],allowedMentions:{roles:roleIds}});
  }catch(err){console.error("[Tracker Presence]",err);}
}

export async function postTrackerEvent(client,guildId,driver,event){
  if(!event)return;
  const allowed=new Set(["job-started","job-delivered","job-cancelled","fuel-stop","fine","rest-stop","crash","toll","ferry","train"]);
  if(!allowed.has(event.type))return;
  try{
    const guild=await client.guilds.fetch(guildId);
    const setup=await ensureTrackerChannels(guild);
    const ch=trackerEventsChannelId?await guild.channels.fetch(trackerEventsChannelId).catch(()=>null):setup.events;
    ...
    switch(event.type){
      ...
      case "train": title="🚆 Train Used";description=`**${who(driver)}** used a train crossing/transport.`;break;
    }
    const e=new EmbedBuilder().setTitle(title).setDescription(description).setTimestamp().setFooter({text:"Sterling Logistics Hidden Tracker Log"});if(fields.length)e.addFields(fields);await ch.send({embeds:[e]});
