...
export async function ensureApprovalSchema(){
  ...
  await db().query(`CREATE TABLE IF NOT EXISTS tracked_job_approvals(
    ...
    INDEX(status,created_at),INDEX(driver_id,created_at))`);
  ready=true;
}

export async function queueTrackedJobForApproval(driverId,data,sessionId){
  ...
  const ref=`tracker:${sessionId}:${origin||""}:${destination||""}:${cargo||""}:${Math.round(revenue)}`;
  try{
    const[r]=await db().execute(`INSERT INTO tracked_job_approvals(driver_id,session_id,reference_key,cargo,origin_city,destination_city,distance_miles,revenue,driver_payment,damage)
      VALUES(?,?,?,?,?,?,?,?,?,?)`,[driverId,sessionId,ref,cargo,origin,destination,miles,revenue,payment,damage]);
    const code=`SL-JA-${String(r.insertId).padStart(5,"0")}`;
    await db().execute("UPDATE tracked_job_approvals SET approval_code=? WHERE id=?",[code,r.insertId]);
    return{queued:true,approvalId:r.insertId,approvalCode:code,status:"pending",payment,revenue};
  }catch(e){
    if(e.code!=="ER_DUP_ENTRY")throw e;
    const[rows]=await db().execute("SELECT id,approval_code,status,driver_payment,revenue FROM tracked_job_approvals WHERE reference_key=? LIMIT 1",[ref]);
    const x=rows[0];return{queued:false,approvalId:x?.id,approvalCode:x?.approval_code,status:x?.status,payment:num(x?.driver_payment),revenue:num(x?.revenue)};
  }
}
...
export async function reviewTrackedApproval(code,decision,reviewer,notes){
  ...
  try{
    ...
    if(decision==="approve"){
      ...
      await conn.execute("INSERT INTO economy_transactions(driver_id,type,amount,category,reference_key,details_json) VALUES(?,?,?,?,?,?)",[a.driver_id,"income",a.revenue,"job_revenue",incomeKey,JSON.stringify({approvalId:a.id,cargo:a.cargo,sourceCity:a.origin_city,destinationCity:a.destination_city})]);
