...
internal sealed class DispatchShell : Form
{
    ...
    private readonly Label _approvalStatus = StatusLabel("Pending tracked deliveries waiting for staff review.");
    ...
    private readonly Label _payoutStatus = StatusLabel("Pending ETS2/ATS payouts created after job approval.");
    ...
    private void BuildApprovals(Control host)
    {
        ...
        var top=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2};top.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));top.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,190));top.Controls.Add(_approvalStatus,0,0);top.Controls.Add(_approvalRefresh,1,0);root.Controls.Add(top,0,0);
        foreach(var c in new[]{Col("Approval","ApprovalCode",120),Col("Driver","Driver",210),Col("Cargo","Cargo",150),Col("Route","Route",220),Col("Miles","Miles",80,"N1"),Col("Revenue","Revenue",100,"C2"),Col("Driver Pay","DriverPayment",100,"C2"),Col("Damage %","Damage",85,"N1"),Col("Created","CreatedAt",160)})_approvalGrid.Columns.Add(c);root.Controls.Add(_approvalGrid,0,1);
        var actions=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=3};actions.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));actions.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,180));actions.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,180));actions.Controls.Add(StatusLabel("Approve releases official stats and creates the driver payout."),0,0);actions.Controls.Add(_approve,1,0);actions.Controls.Add(_decline,2,0);root.Controls.Add(actions,0,2);
    }

    private void BuildPayouts(Control host)
    {
        var root=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=1,RowCount=3};root.RowStyles.Add(new RowStyle(SizeType.Absolute,50));root.RowStyles.Add(new RowStyle(SizeType.Percent,100));root.RowStyles.Add(new RowStyle(SizeType.Absolute,54));host.Controls.Add(root);
        var top=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2};top.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));top.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,190));top.Controls.Add(_payoutStatus,0,0);top.Controls.Add(_payoutRefresh,1,0);root.Controls.Add(top,0,0);
        ...
        var actions=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2};actions.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,100));actions.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute,180));actions.Controls.Add(StatusLabel("Pending payouts are picked up automatically by the driver's Tracker. Retry clears a previous sync error."),0,0);actions.Controls.Add(_payoutRetry,1,0);root.Controls.Add(actions,0,2);
    }
    ...
    {
        try{_approvalRefresh.Enabled=false;var rows=await _api.GetJobApprovalsAsync("pending");_approvalGrid.DataSource=rows;_approvalStatus.Text=rows.Count==0?"No tracked deliveries are waiting for review.":$"{rows.Count} tracked deliver{(rows.Count==1?"y":"ies")} waiting for staff approval.";}catch(Exception ex){_approvalStatus.Text="Job log unavailable • "+ex.Message;}finally{_approvalRefresh.Enabled=true;}
    }

    private async Task RefreshPayoutsAsync()
    {
        try{_payoutRefresh.Enabled=false;var rows=await _api.GetPayoutsAsync("pending");_payoutGrid.DataSource=rows;var errors=rows.Count(x=>!string.IsNullOrWhiteSpace(x.Error));_payoutStatus.Text=rows.Count==0?"No pending payouts.":$"{rows.Count} pending payout{(rows.Count==1?"":"s")}{(errors>0?$" • {errors} need retry":"")}.";}catch(Exception ex){_payoutStatus.Text="Payout monitor unavailable • "+ex.Message;}finally{_payoutRefresh.Enabled=true;}
    }
    ...
    private async Task CancelAsync(){var w=Selected();if(w is null){MessageBox.Show("Select an assignment first.");return;}if(MessageBox.Show($"Cancel {w.WorkCode}?","Sterling Dispatch",MessageBoxButtons.YesNo,MessageBoxIcon.Question)!=DialogResult.Yes)return;try{await _api.CancelAsync(w.WorkCode,"Cancelled in Sterling Dispatch Staff Edition");await RefreshDispatchAsync();}catch(Exception ex){MessageBox.Show(ex.Message);}}
    private async Task ReassignAsync(){var w=Selected();if(w is null||_driver.SelectedItem is not DispatchDriver d){MessageBox.Show("Select an assignment and the new driver.");return;}try{await _api.ReassignAsync(w.WorkCode,d.Id);_message.Text=$"{w.WorkCode} reassigned to {d.SterlingDriverId}";await RefreshDispatchAsync();}catch(Exception ex){MessageBox.Show(ex.Message);}}

    private async Task DecideAsync(string decision)
    {
        if(_approvalGrid.CurrentRow?.DataBoundItem is not DispatchJobApproval a){MessageBox.Show("Select a pending job first.");return;}var verb=decision=="approve"?"approve":"decline";if(MessageBox.Show($"{char.ToUpper(verb[0])+verb[1..]} {a.ApprovalCode} for {a.Driver}?","Sterling Job Approval",MessageBoxButtons.YesNo,MessageBoxIcon.Question)!=DialogResult.Yes)return;try{_approve.Enabled=_decline.Enabled=false;await _api.DecideJobAsync(a.ApprovalCode,decision);await RefreshApprovalsAsync();await RefreshPayoutsAsync();}catch(Exception ex){MessageBox.Show(ex.Message,"Sterling Job Approval",MessageBoxButtons.OK,MessageBoxIcon.Warning);}finally{_approve.Enabled=_decline.Enabled=true;}
    }

    private async Task RetryPayoutAsync()
    {
        if(_payoutGrid.CurrentRow?.DataBoundItem is not DispatchPayout p){MessageBox.Show("Select a pending payout first.");return;}try{_payoutRetry.Enabled=false;await _api.RetryPayoutAsync(p.Id);await RefreshPayoutsAsync();MessageBox.Show($"Payout {p.Id} has been returned to the automatic Tracker sync queue.","Sterling Payout",MessageBoxButtons.OK,MessageBoxIcon.Information);}catch(Exception ex){MessageBox.Show(ex.Message,"Sterling Payout",MessageBoxButtons.OK,MessageBoxIcon.Warning);}finally{_payoutRetry.Enabled=true;}
    }
